...
module rounded_square_test_pattern(x,y,z,r)
{
    ...
    difference()
    {
        union()
        {
            ...
            innerw=w-2*border;
            //color("red")
                translate([border-shiftx,border-shifty,0])
                    cube([innerw,innerh,th]);
        }
        ...
        for (i = [0:test_circles])
        {
            ...
            if (i==6) //filament holder
            {
                //filament container
                filament_r=1.8/2;
                translate([wall+r_hole+r_hole*2.5*i, h+.1+1, filament_r+.2]) //th/2
                    rotate([90,0,0])
                        cylinder(r=filament_r,h=r_hole*2+wall+2+1,center=false);
            }
        }          
        ...
        for(i=[0:wallwide_number-1])
        {
            ...
            translate([x-shift-(cutwide+wallwide)*i,-.1,-.1]) 
                cube([cutwide,cutin+.1,th_outer+.2]);
        }
        
        //overhang
        
        i=wallwide_number-1;
        short_x=(x-2*shift-(cutwide+wallwide_from+wallwide_step*i)*i);
        translate([shift,-.01,-th_outer])
            rotate([0,-asin(2*th_outer/short_x),0])
                cube([short_x,cutin,th_outer]);
        ...
        if (text_type=="raised")
        {
            ...
            translate([ add+add,h-wall/2-fontarea_h,th-font_recessed ])
                rounded_square(w-wall-add,fontarea_h-add,th+10,2);
        }
    ...
    if(test_circles>=1)
        translate([wall+r_hole+r_hole*2.5,h-wall-r_hole,th/2])
            intersection()
            {
                ...
                translate([0,0,-r_hole+th/2*0])
                    sphere(r=r_hole*1.13);
            }
...
{   
    ...
    translate([ w-wall,h-wall-textsize_upper,height ])
        linear_extrude(height=th,convexity = 10)
            text(textstring_Material,size=textsize_upper,font=fontname,halign="right",valign="baseline");  

    translate([ w-wall,h-wall-textsize_upper-line_top_sep,height ])
        linear_extrude(height=th,convexity = 10)
            text(textstring1,size=textsize_lower,font=fontname,halign="right",valign="baseline");  
...
{            
    ...
    difference()
    {
        ...
        write_text_on_top(w,h,th_outer,(edge_tests=="enabled"?wall:round)+2);
        ...
        for (i = [0:len(steps_thickness)-1])
        {
            ...
            translate([ wall*1,wall*stepareadistance,(steps_thickness[i]<0?-.1:steps_thickness[i]+step_thickness_correction) ])
            {
        ...
        {//xxx     if(text_type!="embossed")
            translate([ wall/2,h-wall/2-fontarea_h,th-font_recessed ])
                rounded_square(w-wall,fontarea_h,th+10,round-wall/2);
        }
